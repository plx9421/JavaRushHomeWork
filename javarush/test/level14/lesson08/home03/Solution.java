...
public class Solution
{
    public static void main(String[] args) throws Exception
    {
        ...
        ArrayList<Person> arrayPerson = new ArrayList<Person>(6);

        //тут цикл по чтению ключей, пункт 1
        String name = reader.readLine();
        while (!name.isEmpty()){
            if ("user".equals(name)){
                arrayPerson.add(new Person.User());
            } else
            if ("looser".equals(name)){
                arrayPerson.add(new Person.Looser());
            } else
            if ("coder".equals(name)){
                arrayPerson.add(new Person.Coder());
            } else
            if ("proger".equals(name)){
                arrayPerson.add(new Person.Proger());
            } else break;
