...
//            BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream("C:\\tmp\\33333")))
            ...
            String[] arrayInputLine = sb.toString().split(" ");
            String[] arrayInputLine2 = new String[arrayInputLine.length];
            int a = 0;
            for (int i = 0; i < arrayInputLine.length; i++) {
                if (arrayInputLine == null) continue;
                if ("".equals(arrayInputLine[i])) continue;
                if (" ".equals(arrayInputLine[i])) continue;
                arrayInputLine2[a] = arrayInputLine[i];
                a++;
            }
//todo сервак не принял. на лидирующих пробелах в файле не работает
            //...
            StringBuilder result = getLine(arrayInputLine2);
